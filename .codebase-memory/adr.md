...
**Decision:** Layouts (`_layout.tsx`) cache once per URL *pattern* + the concrete values of the dynamic segments that pattern owns (`kiln:layout:html:v<N>:<pattern>`, suffixed `|<instance-token>` when the pattern is dynamic), shared across every route nested under it, instead of being baked into each route's own cache entry. `cache.deleteLayout(pattern)` invalidates the shared layout in one write — every instance of a dynamic pattern, since the source changed; pass `params` to drop just one.
...
**Own-params amendment (2026-07-28):** the key was originally the pattern string alone, which contradicted the own-params half of the rule — `/projects/:id` had one entry for every project and leaked the first-baked project's chrome everywhere. Params the layout's own pattern owns now enter the key; a descendant page's params deliberately do not (that would destroy the sharing this ADR exists for). A `Live.list` inside a dynamic layout is still pattern-identified — `boot.ts` warns; keep such lists in the page.
