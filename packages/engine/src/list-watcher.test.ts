...
class FakeRedis {
  ...
  async getHtml(route: string) { return this.html.get(route) ?? null; }
  async getJson(route: string) { return this.json.get(route) ?? null; }
  async setHtml(route: string, html: string) { this.html.set(route, html); }
  async setJson(route: string, json: any) { this.json.set(route, structuredClone(json)); }
  async publishPatch(patch: any) { this.published.push(patch); }
}
  ...
  await isolatedWatcher.registerLiveList(
    {
      ...
      renderRows: async () => new Map([["1", '<li data-kiln-key="1"><span data-kiln-field="value">updated</span></li>']]),
    },
    {
      route: "/multi",
      name: "first",
  ...
  await isolatedWatcher.registerLiveList(
    {
      ...
      keyOf: (row: any) => row.id,
      query: () => { throw new Error("second list should not execute"); },
    ...
    },
    {
      ...
      name: "second",
      dependsOn: ["second_events"],
      rows: [{ key: "2", data: { id: "2", value: "unchanged" }, html: "<li>unchanged</li>" }],
    },
  );
  isolatedStore.lists.snapshots.get(key("/multi", "first"))!.stale = true;
  await isolatedWatcher.runOnce();
  assert.deepEqual(isolatedPatches.map((patch) => patch.list), ["first"]);
  assert.equal(
    (isolatedStore.lists.snapshots.get(key("/multi", "second"))!.rows[0].data as any).value,
    "unchanged",
  );

  const redisHtmlPath = path.join(tempDir, "redis.html");
  const redisJsonPath = path.join(tempDir, "redis.json");
  const redisInitialHtml = '<ul data-kiln-list="todos"><li data-kiln-key="1"><span data-kiln-field="status">old</span></li></ul>';
