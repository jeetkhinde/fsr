...
export const tsconfigJson = `{
  ...
  "include": ["src/**/*", "pages/**/*", "api/**/*", "fsr.config.ts"]
}
`;
  ...
  return (
    <html lang="en">
      <head>
        ...
        <title>Fsr.js Application</title>
...
export default function IndexPage() {
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', padding: '2rem', maxWidth: '600px', margin: '0 auto' }}>
      <h1>Welcome to Fsr.js!</h1>
      <p style={{ color: '#666' }}>This is a new page generated by create-fsr.</p>
...
export const actions = {
  async submit(req: any) {
    ...
    return {
      message: \`Hello, \${name || 'stranger'}! Greetings from Elysia + Fsr.js server!\`
    };
  }
};
`;
...
export const fsrFsr = pgTable('fsr_fsr', {
...
export const migrationSql = `-- Create fsr_fsr table
CREATE TABLE IF NOT EXISTS fsr_fsr (
  route TEXT NOT NULL,
  ...
  CONSTRAINT fsr_fsr_pkey PRIMARY KEY (route, slot)
);

-- Invalidation function & trigger utility
CREATE OR REPLACE FUNCTION fsr_notify_change() RETURNS trigger AS $$
BEGIN
  PERFORM pg_notify(
    'fsr_invalidate',
...
async function main() {
  ...
  await startFsr(adapter, config, './pages', fsr);
