...
5. Query Postgres in `load()` through `createKilnSql` (`@kiln/core/sql`), not a plain `new SQL(url)` — it auto-derives live fields' `depends_on` from the tables queried (over-capture is safe; manual `dependsOn` keys still work for row-level precision). Pair with `kiln.config.ts`'s `fsr.triggerTables` + `kiln sync-triggers` (installs the invalidation triggers; `--check` for CI) instead of hand-writing trigger SQL. See `rendering-and-caching.md`.
