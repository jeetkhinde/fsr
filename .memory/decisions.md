...
*   **Known limitations / future work**:
    ...
    *   ~~**Auto-deps is proven at the mechanism layer, not yet end-to-end through a live page.**~~ — **CLOSED**, in two steps. The premise recorded here ("jags-list has zero `Live.value`/`Live.list` usage on any page today") stopped being true with **PR #24**, which gave `apps/jags-list/pages/projects/[id]/activity.tsx` a `Live.list` — though pinned to an explicit `dependsOn: 'activity'`, so an *auto-derived* `depends_on` still wasn't what the page ran on. **PR #32** removed the pin. That page now declares no `dependsOn` at all and depends entirely on the capture Proxy finding `activity` (and `user`, via the join) in the list's own query, with `apps/jags-list/tests/live.integration.test.ts` driving capture → trigger → notify → watcher → Redis → SSE against the real stack. Regressing the capture times that suite out at 20s, so the mechanism itself is the thing under test.
