...
*   **Known limitations / future work**:
    *   **DELETE-driven tombstoning is not owner-scoped.** `FsrWatcher.notifyDelete` → `store.tombstoneDependentRoutes(depKey)` tombstones a route for every user, never just the deleted row's owner — a pre-existing (pre-dating this plan) code path deliberately left unchanged, per this plan's constraint that tombstone behavior must not change. Only INSERT/UPDATE are owner-scoped today. Real gap once an app combines `bake='user'` with a live field depending on a table whose rows get deleted.
    ...
    *   **Dynamic-segment `bake='user'` + live fields**: `bakeByPattern` (the Plan-2-review SSE/snapshot fix above) matches the concrete request route string, not a dynamic pattern (`/users/:id`) — live SSE identity scoping silently falls back to the shared key on such a route. Warned once at runtime (`boot.ts`); not fixed by this plan.
