...
| A dynamically-interpolated table name — ``sql`SELECT * FROM ${sql(name)}` `` | **Invisible to auto-deps** — the name reaches the template as a bound `?`, so there is no table to parse. Warned once per query shape | Give the field an explicit `dependsOn` / `Live.value(x, [...])` |
