...
- **What it captures**: `page-render.ts` runs every request's real `load()` call inside `withDepCapture` (`@kiln/core/sql`); `createKilnSql` records the table names parsed out of each tagged-template query into that capture scope. When a page's live fields are persisted, the observed tables are unioned into each field's `depends_on` — an explicit `Live.value(x, ['sessions'])` still keeps `'sessions'`, plus whatever was auto-observed.
- **A `Live.list` captures its own query, separately.** Scalar fields get the tables `load()` touched; a list gets the tables *its own* `query` touched, captured in its own scope when the list is materialized. This matters because `initial` is optional — a list that never runs a query inside `load()` still gets correct dependencies — and because two lists on one page do not contaminate each other's `depends_on`. Explicit `dependsOn` is preserved and unioned exactly as it is for scalars.
- **A layout's `load()` is captured separately from its page's.** Layout scalar live fields are registered under the page's concrete route (that is what the browser subscribes with) but take *their own* layout's observed tables as auto-deps — a page's tables never become a layout field's dependency, or the reverse. See [live-and-islands.md](live-and-islands.md#liveprop-in-a-layout).
- **A list with no dependencies at all warns.** If nothing was declared *and* nothing was captured — a query built with a plain `new SQL(url)`, or a dynamically-interpolated table name — the list would silently stop tracking writes, so Kiln logs a warning naming the route and list. With `revalidate: false` such a list never updates; otherwise it degrades to the revalidate timer (~300s by default) instead of reacting to writes.
- **Every handle the client hands out captures too.** `sql.begin(async tx => ...)`, a `tx.savepoint(...)` nested inside it, `sql.unsafe('SELECT ...')` and `await sql.reserve()` all record their tables. Each was a silent under-capture gap until 2026-08-01: the query ran through bun's own object rather than the instrumented one, so from the wrapper's point of view no query happened at all — nothing captured, nothing warned. A transaction still rolls back normally; only the handle is substituted.
- **Table-level, not row-level.** Table names are parsed from `FROM`/`JOIN`/`INTO`/`UPDATE` with a best-effort regex — `WHERE` predicates aren't inspected. Over-capture (an extra table the query touched but didn't strictly need) only costs one extra revalidation check, and the regex only ever over-matches, never misses a table it can see — the safe failure direction. The one remaining way to under-capture is a **dynamically-interpolated table name** (``sql`SELECT * FROM ${sql(name)}` ``), which reaches the template as a bound `?` and leaves nothing to parse; that case warns once per query shape. For row-scoped precision (`contacts:id=42`), pass a manual dependency key — see [live-and-islands.md](live-and-islands.md#manual-dependency-keys-the-row-level-escape-hatch).
