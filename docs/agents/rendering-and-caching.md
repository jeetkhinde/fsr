...
- **Table-level, not row-level.** Table names are parsed from `FROM`/`JOIN`/`INTO`/`UPDATE` with a best-effort regex — `WHERE` predicates aren't inspected. Over-capture (an extra table the query touched but didn't strictly need) only costs one extra revalidation check; it can never under-invalidate, so the loose regex is the safe failure direction. For row-scoped precision (`contacts:id=42`), pass a manual dependency key — see [live-and-islands.md](live-and-islands.md#manual-dependency-keys-the-row-level-escape-hatch).
