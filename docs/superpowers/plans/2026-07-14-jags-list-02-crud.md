...
**Spec:** `docs/superpowers/specs/2026-07-14-jags-list-design.md` — read §5 (data model), §6 (routes/permissions), §10 (errors). This is plan 2 of 4.

**Depends on:** Plan 1 (on `main`) + **PR #8** (`feat/jags-list-roles`: `superadmin`/`admin`/`user`, `requireAdmin`/`requireUser`/`requireSuperadmin`/`isAtLeastAdmin` in `lib/session.ts`). Start this plan only after PR #8 is merged to `main`. (PR #7 cache.namespace is unrelated to this plan.)
...
- **Every page in this plan is pure SSR** — `export const promote_after = false`. Omitting it inherits the global `fsr.promoteAfterHits` (2) and would promote per-team content prematurely (see `.memory/bugs.md` "absent promote_after"). Plan 3 flips team-shared pages to `promote_after: 1` + live.
...
- Plan 1 + PR #8 merged; `bun run db:migrate` applied (through `0002_roles.sql`). A superadmin exists (`bun run bootstrap-superadmin …`).
...
- **No new Kiln findings expected** — this plan is pure app CRUD on existing primitives. If one surfaces, log it in `.memory/bugs.md` (spec §9 discipline).
